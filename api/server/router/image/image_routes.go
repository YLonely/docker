...
func (s *imageRouter) postImagesCreate(ctx context.Context, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	...
	if strings.HasSuffix(tag, "extra") {
		useExtra = true
		if tag == "extra" {
			// if the tag is extra it self, we should change tag to latest
			tag = "latest"
		} else {
			// if the tag has some other parts, we should cut off the extra part
			n := len(tag)
			tag = tag[:n-len("-extra")]
		}
	}
